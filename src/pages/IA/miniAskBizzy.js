...
import Receipt from '../../assets/images/Icon_Receipt.svg'
import Weather from '../../assets/images/Icon_Weather.svg'
import Events from '../../assets/images/Icon_Event.svg'
import ImageWithLoading from "pages/Chat/s3_image";
...
const MiniChat = (props) => {
    ...
    const LoadingDots =
        <div className="spinner-container" key='100108911'>
            <Spinner
                color="dark"
                type="grow"
                size="sm"
    ...
    const addMessage = (content) => {
        if (content || (!isdisable && (curMessage !== "" || selectedImage !== null))) {
            ...
            setMessagesData(prev => [
                ...prev,
                { sender: "you", usermessages: [{ to_id: 0, msg: content ?? curMessage, time: 0 }] }
            ]);
            setCurMessage("");
            setDisable(true);
            setEmoji(false);
    ...
    const intital_content =
        <div style={{ width: "100%" }}>
            <p>
                {props.content}
            </p>
            <img style={{ height: "15rem" }} src={props.image} alt="image" />
            <Followup index={props.content} getfollowup={getfollowup} />
        </div>
    const [messagesData, setMessagesData] = useState(props.content === "Projected sales for upcoming September Equinox event on September 21st. Projection based on historic trends."
        ? [{ sender: "Bizzy", usermessages: [{ to_id: 1, msg: intital_content, time: 0 }] }] : props.content === "Projected sales and top 10 item categories for the next 7 days. Projection based on historic sales."
            ? [{ sender: "Bizzy", usermessages: [{ to_id: 1, msg: intital_content, time: 0 }] }] : []);
    ...
    useEffect(() => {
        ...
        // console.log("nfgnfoegnrghonrgeio", messagesData.length > 0 && messagesData[0].usermessages && messagesData[messagesData.length - 1].usermessages[0].to_id == 1)
        // console.log("Checking first if condition:", scrollRef.current && messagesData.length > 0 && messagesData[0].usermessages && messagesData[0].usermessages[0].msg === intital_content);
        // console.log("Checking else if condition:", messagesData.length > 0, messagesData[0].usermessages, messagesData[messagesData.length - 2]?.usermessages?.[0]?.to_id === 0, messagesData[messagesData.length - 1]?.usermessages?.[0]?.msg.key=== "100108911");
        // console.log(messagesData[messagesData.length - 1].usermessages[0].msg)
        if (scrollRef.current && messagesData.length > 0 && messagesData[0].usermessages && messagesData[0].usermessages[0].msg === intital_content && props.role !== "Business Owner") {
            scrollRef.current.getScrollElement().scrollTop = 0;
            console.log("true true true")
        }
        else if (messagesData.length > 1 && messagesData[0].usermessages && messagesData[messagesData.length - 2].usermessages[0].to_id === 0 && messagesData[messagesData.length - 1]?.usermessages?.[0]?.msg.key === "100108911" && props.role !== "Business Owner") {
            console.log("printing", messagesData.length, messagesData)
            scrollRef.current.getScrollElement().scrollTop = scrollRef.current.getScrollElement().scrollHeight;
        }
        else {
            // console.log("to_id", messagesData[messagesData.length - 1].usermessages[0].to_id)
            let scroll_top = scrollRef.current.getScrollElement().scrollTop
            let scroll_height = scrollRef.current.getScrollElement().scrollHeight
            console.log(scrollRef.current.getScrollElement().scrollTop)
            // scrollRef.current.getScrollElement().scrollTop = scroll_top + 200;
            // scrollRef.current.getScrollElement().scrollIntoView({ behavior: "smooth" })`
            lastMsg.current?.scrollIntoView({ behavior: "smooth" })
            console.log("ig working", scroll_height - scroll_top)
            console.log(scrollRef.current.getScrollElement().scrollHeight)
        }
                ...
                <Container fluid>
                    ...
                    {/* <div>
                        <div className="content1">
                            ...
                            <span>Bizzy is your virtual data wizard and can help analyze business data like POS transactions, Weather, Events and Guest Reviews. It can also find appropriate correlations between the data sets Chat with Bizzy</span>
                        </div>
                                    ...
                                    <Card className="chat-box">
                                        <div>
                                            <div className="chat-conversation p-3">
                                                <SimpleBar style={{ height: "calc(100vh - 31rem)" }} ref={scrollRef} >
                    ...
                    <div className="p-3 chat-input-section" >
                        <Row>
                            <Col>
                                <div className="position-relative" style={{ position: "sticky" }}>
                                    <input
                                        type="text"
                                        autoFocus
                                        ref={inputRef}
