...
const LabourTrends = (props) => {
  let user_id = JSON.parse(localStorage.getItem("authUser")).uid;
  let percentChange = props.value.diff
  let state = <span>
    {" "}
      ...
      <div className="custom-card">
        ...
        </div>
        {
          JSON.stringify(props.value) === "{}" ?
            ...
            :
            <React.Fragment>
              {/* <h3>{props.value.uniq_user_this_week}</h3>
              <p className="text-muted persentage-style" style={{bottom: "1.5rem"}}>
                
                {state}<br/>From previous week
              </p> */}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', whiteSpace: 'nowrap' }}>
                <h3 style={{ fontSize: "calc(100% + 0.5rem)" }}>{props.value.uniq_user_this_week}</h3>
                {percentChange && (
